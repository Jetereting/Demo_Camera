...
/**
 * HTTP连接装置 HTTP connection to device
 */
public class HttpConnector {
	...
	private String connect() {
		HttpURLConnection postConnection = createHttpConnection("POST",
	...
	/**
	 * 获取设备的存储信息 Acquire storage information of device
	 * 
	...
	/**
	 * 获取设备信息 Acquire device information
	 * 
	 * @return Device information
	 */
	public DeviceInfo getDeviceInfo() {
		HttpURLConnection getConnection = createHttpConnection("GET",
				"/osc/info");
		String responseData;
	...
	/**
	 * 获取设备上的媒体文件列表 Acquire list of media files on device
	 * 
	...
	public ArrayList<ImageInfo> getList() {
		...
		for (int continuation = 0; continuation < 3; continuation++) {
			ArrayList<ImageInfo> receivedImageInfo = getListInternal(10,
					mContinuationToken);
	...
	/**
	 * 获取媒体文件列表（项目数量有限） Acquire media file list (limited number of items)
	 * 
	 * @param maxReceiveEntry
	 *            Maximum number of files that can be acquired at once
	 * @param token
	 *            Set the previously acquired token to continue. Set null if
	 *            acquiring for the first time.
	 * @return List of specified number of media files
	 */
	private ArrayList<ImageInfo> getListInternal(int maxReceiveEntry,
			String token) {
		HttpURLConnection postConnection = createHttpConnection("POST",
				"/osc/commands/execute");
		JSONObject input = new JSONObject();
			...
			if (status.equals("done")) {
				...
				for (int index = 0; index < entrySize; index++) {
					...
					try {
						...
						imageInfo
								.setFileFormat(ImageInfo.FILE_FORMAT_CODE_EXIF_MPEG);
					} catch (JSONException e) {
						imageInfo
								.setFileFormat(ImageInfo.FILE_FORMAT_CODE_EXIF_JPEG);
					}
...
//	        outStream.close();  
	 ...
	 * Take photo
	 * <p>
	 * After shooting, the status is checked for each
	 * {@link HttpConnector#CHECK_STATUS_PERIOD_MS} and the listener notifies
	 * you of the status.
	 * 
	 * @param listener
		...
		try {
			...
			if (status.equals("inProgress")) {
				...
				mCheckStatusTimer.scheduleAtFixedRate(capturedTimerTask,
						CHECK_STATUS_PERIOD_MS, CHECK_STATUS_PERIOD_MS);
	...
	/**
	 * 检查静止图像的拍摄状态 Check still image shooting status
	 * 
	 * @param commandId
	...
	/**
	 * 获取指定图像的原始数据 Acquire raw data of specified image
	 * 
	 * @param fileId
	 *            File ID
	 * @param listener
	...
	/**
	 * 获得实时查看流 Acquire live view stream
	 * 
	...
	/**
	 * 文件删除状态检查类 Status check class for file deletion
	 */
	...
	/**
	 * 获取当前设置的射击大小 Acquire currently set shooting size
	 * 
	...
	/**
	 * 设置静止图像为拍摄模式 Set still image as shooting mode
	 * 
	 * @param sessionId
	 *            Session ID
	...
	/**
	 * 检查设备状态的更新 Check for updates to device status
	 * 
	...
	/**
	 * 生成连接目标地址 Generate connection destination URL
	 * 
	 * @param path
	 *            Path
	 * @return URL
	...
	/**
	 * 生成的HTTP连接 Generate HTTP connection
	 * 
	 * @param method
	 *            Method
	 * @param path
	...
	/**
	 * 将输入流转换为字符串 Convert input stream to string
	 * 
	 * @param is
	 *            InputStream
	 * @return String
	...
	public void getThumb2(String fileId) {
		HttpURLConnection postConnection = createHttpConnection("POST",
				"/osc/commands/execute");
		JSONObject input = new JSONObject();
		Bitmap thumbnail = null;

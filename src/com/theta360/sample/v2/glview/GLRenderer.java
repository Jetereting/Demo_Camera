...
public class GLRenderer implements Renderer {
	 ...
	 */
	public GLRenderer() {
		mEastShell = new UVSphere(Constants.TEXTURE_SHELL_RADIUS,
				Constants.SHELL_DIVIDES, true);
		mWestShell = new UVSphere(Constants.TEXTURE_SHELL_RADIUS,
				Constants.SHELL_DIVIDES, false);
	...
	public void onDrawFrame(final GL10 gl) {
		...
		mCameraDirectionX = (float) (Math.cos(mRotationAngleXZ) * Math
				.cos(mRotationAngleY));
		mCameraDirectionZ = (float) (Math.sin(mRotationAngleXZ) * Math
				.cos(mRotationAngleY));
		...
		if (mTextureUpdate && null != mTexture
				&& !mTexture.getPhoto().isRecycled()) {
			Log.d("", "load texture1");
			loadTexture(mTexture.getPhoto());
			mTextureUpdate = false;
		}

		Matrix.setLookAtM(mViewMatrix, 0, mCameraPosX, mCameraPosY,
				mCameraPosZ, mCameraDirectionX, mCameraDirectionY,
				mCameraDirectionZ, 0.0f, 1.0f, 0.0f);
		Matrix.perspectiveM(mProjectionMatrix, 0, mCameraFovDegree,
				mScreenAspectRatio, Z_NEAR, Z_FAR);
		...
		GLES20.glUniformMatrix4fv(mProjectionMatrixHandle, 1, false,
				mProjectionMatrix, 0);
	...
	public void onSurfaceChanged(final GL10 gl, final int width,
		...
		mScreenAspectRatio = (float) width
				/ (float) (_height == 0 ? 1 : _height);
	...
	public void onSurfaceCreated(final GL10 gl, final EGLConfig config) {
		...
		mProjectionMatrixHandle = GLES20.glGetUniformLocation(program,
				"uProjection");
	 ...
	 * 
	 * @param ratio
	 *            Scale value: Zoom in process performed if the value is 1.0 or
	 *            more; zoom out process is performed if the value is less than
	 *            1.0
	 */
	public void scale(float ratio) {

		if (ratio < 1.0) {
			mCameraFovDegree = mCameraFovDegree
					* (Constants.SCALE_RATIO_TICK_EXPANSION);
			if (mCameraFovDegree > Constants.CAMERA_FOV_DEGREE_MAX) {
				mCameraFovDegree = Constants.CAMERA_FOV_DEGREE_MAX;
			}
		} else {
			mCameraFovDegree = mCameraFovDegree
					* (Constants.SCALE_RATIO_TICK_REDUCTION);
	...
	public void loadTexture(final Bitmap texture) {
		...
		for (int textureIndex = 0; textureIndex < 2; textureIndex++) {
			...
			Bitmap dividedBitmap = Bitmap.createBitmap(bitmap,
					(dividedWidth * textureIndex), 0, dividedWidth,
					bitmap.getHeight());

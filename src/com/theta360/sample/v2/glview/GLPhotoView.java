				...
				new GestureDetector.OnGestureListener() {
					...
					public boolean onScroll(MotionEvent e1, MotionEvent e2,
						...
						if ((Math.abs(distanceX) > SWIPE_MAX_OF_PATH_X)
							...
							float diffX = distanceX
									/ Constants.ON_SCROLL_DIVIDER_X;
							float diffY = distanceY
									/ Constants.ON_SCROLL_DIVIDER_Y;
					...
					public boolean onFling(MotionEvent e1, MotionEvent e2,
						...
						mScroller.fling((int) e2.getX(), (int) e2.getY(),
								(int) velocityX, (int) velocityY, 0,
							...
							public void run() {
								if (mRotateInertia == RotateInertia.INERTIA_0) {
									...
									if (mRotateInertia == RotateInertia.INERTIA_50) {
										diffX = diffX
												/ Constants.ON_FLING_DIVIDER_X_FOR_INERTIA_50;
										diffY = diffY
												/ Constants.ON_FLING_DIVIDER_Y_FOR_INERTIA_50;
									} else {
										diffX = diffX
												/ Constants.ON_FLING_DIVIDER_X_FOR_INERTIA_100;
										diffY = diffY
												/ Constants.ON_FLING_DIVIDER_Y_FOR_INERTIA_100;
									}
									mRenderer.rotate((float) -diffX,
											(float) diffY);

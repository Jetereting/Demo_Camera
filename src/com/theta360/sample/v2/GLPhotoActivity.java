...
/**
 * 显示照片对象作为一个球体的活动 Activity that displays photo object as a sphere
 */
@SuppressLint("NewApi")
public class GLPhotoActivity extends Activity implements
		ConfigurationDialog.DialogBtnListener {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		setContentView(R.layout.ricoh_activity_glphoto);
		...
		ByteArrayInputStream inputStreamThumbnail = new ByteArrayInputStream(
				byteThumbnail);
		Drawable thumbnail = BitmapDrawable.createFromStream(
				inputStreamThumbnail, null);
	...
	private class LoadPhotoTask extends AsyncTask<Void, Object, ImageData> {
		...
		protected ImageData doInBackground(Void... params) {
			try {
				...
				ImageData resizedImageData = camera.getImage(fileId,
		...
		protected void onPostExecute(ImageData imageData) {
			if (imageData != null) {
				...
				Bitmap __bitmap = BitmapFactory.decodeByteArray(dataObject, 0,
						dataObject.length);
				...
				logViewer.append("<Angle: yaw=" + yaw + ", pitch=" + pitch
						+ ", roll=" + roll + ">");

				mTexture = new Photo(__bitmap, yaw, pitch, roll);
				//SD
				SharedPreferences userinfo = GLPhotoActivity.this
						.getSharedPreferences("userinfo",
								Context.MODE_PRIVATE);
				FileUtils.saveBitmap(mTexture.getPhoto(), userinfo.getString("telephone", "")+"_"+System.currentTimeMillis()+"");
				if (null != mGLPhotoView) {
					mGLPhotoView.setTexture(mTexture);
				}
			} else {
	 ...
	 *            true is to refresh list after closing this activity, otherwise
	 *            is not to refresh
	 */
	public static void startActivityForResult(Activity activity,
			String cameraIpAddress, String fileId, byte[] thumbnail,
